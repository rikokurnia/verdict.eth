...
const GradientWaves = ({
  ...
  useEffect(() => {
    ...
    const loop = t => {
      elapsed += Math.min(t - last, 50) * 0.001;
      last = t;
    ...
    const onMotion = () => { pausedRef.current ? tryStop() : tryStart(); };
    container.addEventListener('waves-motion', onMotion);
    ...
    return () => {
      ...
      container.removeEventListener('waves-motion', onMotion);
  ...
  useEffect(() => {
    ...
    containerRef.current?.dispatchEvent(new Event('waves-motion'));
  }, [paused]);
